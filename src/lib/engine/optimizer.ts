 ...
 */
export function cmaes(f: (params: number[]) => number, start: number[], options: CmaesOptions = {}): number[] {
	...
	const cmu_val = Math.min(1 - c1, (2 * (muEff - 2 + 1 / muEff)) / ((d + 2) * (d + 2) + muEff));
