<script lang="ts">
	...
	const starsPromise = fetch('/stars.json', { signal: starsLoadSignal })
		...
		.catch((err) => {
			if (pageDisposed || starsLoadSignal.aborted || (err instanceof DOMException && err.name === 'AbortError')) {
				return;
			}
	...
	function clearInputRejectTimer() {
		if (inputRejectTimer !== null) {
			clearTimeout(inputRejectTimer);
			inputRejectTimer = null;
		}
	}
	...
	function updateConstellationColor(index: number, color: string) {
		...
		colorUpdateRaf = requestAnimationFrame(() => {
			starField?.redrawConstellations(
				constellations.map((c) => c.result),
	...
	async function handleStarFieldReady() {
		...
		if (pendingShareState) {
			...
			requestAnimationFrame(() => {
				...
				focusedIndex = shareState.focusedIndex >= 0 ? shareState.focusedIndex : constellations.length - 1;
				showInput = false;
	...
	function handleRerollResult(text: string, result: MatchResult) {
		const entry = makeEntry(text, result);
		constellations = [...constellations.slice(0, rerollIndex), entry, ...constellations.slice(rerollIndex + 1)];
	...
	function applyDragAction(action: DragAction, star: Star) {
		...
		if (!entry) return;
		const newPairs = entry.result.pairs.map((p) => (p.nodeIndex === action.nodeIndex ? { ...p, star } : { ...p }));
	...
	function handleClickOutsideSettings(e: PointerEvent) {
		...
		if (
			(selectedStar || selectedConstellation) &&
			!starClickedThisTick &&
			!target.closest('.star-panel') &&
			!target.closest('.star-search-container')
		) {
			selectedStar = null;
			selectedConstellation = null;
			selectionHistory = [];
	...
	function handleGlobalKeydown(e: KeyboardEvent) {
		...
		if (e.key === 'Escape') {
			...
			if (showInput && constellations.length > 0) {
				cancelAddMore();
				return;
			}
			if (aboutOpen) {
...
<main class="app" aria-label="Written in the Stars" onpointerdown={handleClickOutsideSettings}>
	...
	<div class="top-bar">
		<div class="top-bar-buttons">
				...
				{#if settingsOpen}
					<div class="settings-panel" role="menu">
						...
						<button
							...
							onclick={handleToggleMonoColor}
							role="menuitemcheckbox"
							aria-checked={!monoColor}
						>
							<svg
								viewBox="0 0 24 24"
								width="16"
						...
						<button
							class="settings-item"
							class:active={showSun}
							onclick={handleToggleSun}
							role="menuitemcheckbox"
							aria-checked={showSun}
						>
							<svg
								viewBox="0 0 24 24"
								width="16"
						...
						<button
							class="settings-item"
							class:active={shootingStars}
							onclick={handleToggleShootingStars}
							role="menuitemcheckbox"
							aria-checked={shootingStars}
						>
							<svg
								viewBox="0 0 24 24"
								width="16"
			...
			<button class="top-bar-btn" onclick={() => (aboutOpen = !aboutOpen)} aria-label="About this site">
				<svg
					viewBox="0 0 24 24"
					width="18"
					height="18"
				...
				<input
					...
					onkeydown={(e) => {
						...
						if (e.key === 'ArrowDown') {
							...
							handleSearchSelect(results[searchHighlight]);
							searchInputEl?.blur();
		...
		<div class="star-panel" role="region" aria-label="Star info" onpointerdown={(e) => e.stopPropagation()}>
			...
			{#if selectionHistory.length > 0}
				<button class="star-panel-back" onclick={popSelection} aria-label="Go back">
					...
					{selectionHistory[selectionHistory.length - 1].constellation?.name ||
						selectionHistory[selectionHistory.length - 1].star?.name ||
						'Back'}
				</button>
			{/if}
				...
				{#if starConsts.length > 0}
					<div class="star-panel-constellations">
						...
						{#each starConsts as c, i}
							<button
								class="star-panel-constellation-link"
								onclick={() => {
									pushSelection();
			...
			{:else if selectedConstellation}
				...
				<div class="star-panel-rows">
					...
					{#if cStars.length > 0}
						<div class="star-panel-row">
							...
							<span class="star-panel-value"
								>{cStars[0].name || `HIP ${cStars[0].hip}`} ({cStars[0].mag.toFixed(1)})</span
							>
						</div>
					{/if}
				</div>
				{#if constellationWikiUrls[selectedConstellation.name]}
					<a
						class="star-panel-link"
						href={constellationWikiUrls[selectedConstellation.name]}
						target="_blank"
		...
		<div class="about-backdrop" role="presentation" onpointerdown={() => (aboutOpen = false)}>
			...
			>
				...
				<div class="about-footer">
					...
					<a
						class="about-footer-neal"
						href="https://neal.fun/constellation-draw/"
						target="_blank"
						rel="noopener noreferrer"
					>
						Inspired by
								...
								<g id="neal-art-about" transform="matrix(0.982301,0,0,1,0,216.559)">
									...
									<g clip-path="url(#_clip1_about)">
										<g transform="matrix(1.01802,0,0,1,0,0.440917)">
											<path
												d="M179.26,84.007C176.96,81.747 176.07,76.927 178.18,74.297C183.16,68.077 191.43,71.877 192.24,79.047C192.69,83.027 191.63,85.037 187.57,85.707C184.85,86.157 181.33,86.037 179.26,84.007Z"
												fill="currentColor"
												fill-rule="nonzero"
											/>
										</g>
										<g transform="matrix(1.01802,0,0,1,714.649,-189.078)">
											<g transform="matrix(1,0,0,1,-701,190)">
												<path
													d="M36.15,58.807L36.61,57.157C39.59,41.047 42.28,24.867 43.46,8.487C44.21,5.597 51.69,4.367 52.79,7.027C53.24,8.127 52.12,12.007 51.88,13.397C48.49,33.287 46.14,53.237 43.68,73.257C43.43,75.317 42.85,81.757 41.67,83.057C38.98,86.017 35.48,85.057 33.83,81.667L17.14,25.417L9.16,83.377C6.97,88.437 0.27,89.607 0,82.717C4.33,61.707 7.01,40.387 10.93,19.297C11.62,15.597 12.2,11.117 13.22,7.567C13.73,5.797 14.49,4.407 16.5,4.207C20.87,3.787 22.19,8.187 23.18,11.537C26.04,21.207 27.96,31.377 30.66,41.127C32.31,47.077 34.33,52.897 36.14,58.797L36.15,58.807Z"
													fill="currentColor"
													fill-rule="nonzero"
												/>
											</g>
										...
										<g transform="matrix(1.01802,0,0,1,639.243,-186.628)">
											<g transform="matrix(0.999542,-0.0302536,0.0302536,0.999542,-630.24,198.142)">
												<path
													d="M319.41,58.807C319.76,58.847 319.566,58.416 319.54,58.217C317.444,41.903 317.26,24.509 314.771,7.958C314.582,2.21 322.107,0.507 323.361,7.958C323.614,9.462 323.421,11.498 323.561,13.108C325.291,31.808 326.77,52.077 329.39,70.687C329.89,74.277 331.17,78.857 331.22,82.347C331.28,87.267 326.05,87.197 323.68,83.967L293.77,24.697L299.68,83.817C298.31,90.637 291.65,89.357 290.66,82.867C289.77,77.067 290.08,70.527 289.71,64.627C288.74,48.877 287.07,33.167 286.21,17.407C286.02,13.947 285.52,9.297 285.65,5.987C285.81,1.677 290.35,2.607 292.57,4.777C294.47,6.637 297.75,14.267 299.09,17.067C302.53,24.257 305.62,31.617 309.1,38.777C312.38,45.537 316.01,52.117 319.41,58.817L319.41,58.807Z"
													fill="currentColor"
													fill-rule="nonzero"
												/>
											</g>
											<g transform="matrix(1,0,0,1,-628,188.79)">
												<path
													d="M238.84,2.876C243.11,2.816 243.71,5.427 244.05,8.727C245.92,27.367 241.94,49.257 247.28,67.287C248.85,72.607 253.73,81.187 260.27,76.217C267.31,70.857 267.34,51.607 267.66,43.397C268.03,33.757 268.47,23.717 267.48,14.077C267.2,11.317 265.86,7.826 266.41,5.346C266.73,3.906 269.43,3.316 270.68,3.386C275.68,3.686 275.63,6.157 275.96,10.017C276.86,20.557 276.57,31.357 276.32,41.917C275.98,56.637 277.09,89.297 253.79,84.757C238.34,81.747 237.86,63.707 236.86,51.147C235.82,38.007 235.24,24.847 235.02,11.667C234.98,9.147 234.58,8.216 234.82,5.666C235.01,3.666 237.03,2.896 238.84,2.866L238.84,2.876Z"
													fill="currentColor"
													fill-rule="nonzero"
												/>
											</g>
				...
				<div class="constellation-card" class:focused={i === focusedIndex && !showInput}>
					...
					>
						<div
							...
							style="color: {entry.color}; text-shadow: -1px -1px 2px rgba(0,0,0,0.9), 1px -1px 2px rgba(0,0,0,0.9), -1px 1px 2px rgba(0,0,0,0.9), 1px 1px 2px rgba(0,0,0,0.9), 0 0 8px rgba(0,0,0,0.6), 0 0 14px {entry.color}70, 0 0 28px {entry.color}35;"
						>
							{entry.name}
						</div>
				...
				<div class="result-actions" role="toolbar" aria-label="Constellation actions">
					...
					<div class="save-image-container">
						<button
							...
							aria-label="Save constellation as image"
							aria-expanded={saveMenuOpen}>Save image</button
						>
						{#if saveMenuOpen}
							<div class="save-image-menu" role="menu">
								<button class="save-image-option" role="menuitem" onclick={() => handleSaveImage()}
									>Current window</button
								>
								<button class="save-image-option" role="menuitem" onclick={() => handleSaveImage(1920, 1080)}
									>1920 × 1080 (HD)</button
								>
								<button class="save-image-option" role="menuitem" onclick={() => handleSaveImage(2560, 1440)}
									>2560 × 1440 (2K)</button
								>
								<button class="save-image-option" role="menuitem" onclick={() => handleSaveImage(3840, 2160)}
									>3840 × 2160 (4K)</button
								>
							</div>
						{/if}
