...
const CONSTELLATION_OUTPUT = join(
  __dirname,
  "..",
  "static",
  "constellation-wiki.json"
);
...
async function main() {
  ...
  const existingStars: Record<string, WikiResult> = existsSync(STAR_OUTPUT)
    ...
    : {};
  const existingConstellations: Record<string, WikiResult> = existsSync(
  ...
  for (const star of newStars) {
    ...
    if (hit && !hit.url.includes("_(star)")) {
      if (!looksLikeStar(hit.extract)) {
        console.log(
          `SKIP (not a star article: "${hit.extract.slice(0, 60)}...")`
        );
  ...
  const newConstellations = constellationNames.filter(
    (n) => !existingConstellations[n]
  );
  ...
  writeFileSync(
    CONSTELLATION_OUTPUT,
    JSON.stringify(constellationResults, null, 2)
  );
